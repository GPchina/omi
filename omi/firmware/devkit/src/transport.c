...
//
// P15h detour: spectral VAD (FFT + 4 features: voice-band share / high-freq
// share / harmonic peaks / temporal pulse) was tried. PoC on Omi real
// recordings + synthesised background noises looked promising (TV wakeup
// 39% -> 1%, white / fan / HVAC / cough 0%), but the user's real-world
// test showed phone-speaker playback (heavily smoothed in time domain by
// the speaker's tiny driver) did not trigger even at close range. The
// temporal pulse ratio f4 stays low on phone audio because the speaker
// compresses syllable transients. Single-mic omnidirectional + no
// reference signal = any subtle-feature rule has a non-trivial miss rate.
//
// P15i (rollback to conservative energy VAD, route A): manual button is the
// primary recording path. Auto-VAD is now a "near-field loud speech" assist
// only - the user has to be close, speak loudly, AND sustain >600 ms before
// REC_AUTO latches. SNR floor doubled (3 -> 5) so soft phone audio and room
// background cannot cross it. Debounce lengthened to 6 frames (600 ms) so
// neither continuous TV/fan/HVAC nor phone-playback speech bursts (which are
// usually <500 ms) can sustain it. Quiet boot + correct calibration remains;
// no PC tuning needed.
#define VAD_CALIB_BLOCKS 20        // 20 x 100ms = 2s calibration window
#define VAD_SNR_RATIO 5            // P15i: was 3 (P15g), 4 (P15e); voice >= 5x noise (~ +14 dB)
#define VAD_MIN_THRESHOLD 500      // P15i: was 250 (P15g), 400 (P15e); absolute floor doubled
...
// P15i: 6 frames (600 ms). P15g used 4 (400ms), P15e used 2 (200ms). 600 ms
// is past the natural duration of any TV / fan / HVAC sustained-noise
// segment AND past a single phone-playback speech burst (~200-400 ms), so
// those sources cannot sustain it. Real user speech sustains well past 600
// ms in normal conversation.
#define VAD_START_DEBOUNCE_FRAMES 6
